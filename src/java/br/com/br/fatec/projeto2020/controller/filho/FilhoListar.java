 ...
 * @author Luis Silva
 */
@WebServlet(name = "FilhoListar", urlPatterns = {"/FilhoListar"})
public class FilhoListar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            request.setAttribute("filhos", dao.listar());
            request.getRequestDispatcher("/cadastros/Filho/filho.jsp")
                    .forward(request, response);
            
        } catch (Exception ex){
            System.out.println("Problemas no Servlet ao Listar"
                    + " Fornecedores! Erro: " + ex.getMessage());
        }
    }
