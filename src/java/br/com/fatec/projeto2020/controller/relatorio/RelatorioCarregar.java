 ...
 * @author Luis Silva
 */
@WebServlet(name = "RelatorioCarregar", urlPatterns = {"/RelatorioCarregar"})
public class RelatorioCarregar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("relatorio",dao.carregar(idRelatorio));
            request.getRequestDispatcher("/cadastros/Relatorio/relatorioCadastrar.jsp")
                    .forward(request, response);
              
        } catch (Exception ex) {
            System.out.println("Problemas ao Carregar dados do Relatorio! Erro: " 
                    + ex.getMessage());
        }
    }
